...
address_objects['.$Count.'-1] = {
	...
	"LatLng":['.$Location->MapLatitude.','.$Location->MapLongitude.']
	    ...
	    public function ServiceAreasForm()
	    {
	        ...
	        if ($this->getPointer()->isFieldEnabled('FirstName')) {
	            $fields->push(new TextField("FirstName", "First Name"));
	        }
	        if ($this->getPointer()->isFieldEnabled('LastName')) {
	            $fields->push(new TextField("LastName", "Last Name"));
	        }
	        if ($this->getPointer()->isFieldEnabled('Address')) {
	            $fields->push(new TextField("Address", "Address"));
	        }
	        if ($this->getPointer()->isFieldEnabled('Address2')) {
	            $fields->push(new TextField("Address2", "Address (line 2)"));
	        }
	        if ($this->getPointer()->isFieldEnabled('City')) {
	            $fields->push(new TextField("City", "City"));
	        }
	        if ($this->getPointer()->isFieldEnabled('State')) {
	            $fields->push(new DropdownField("State", "State", array(""=>"State") + FormUtilities::GetStates(), "PA"));
	        }
	        if ($this->getPointer()->isFieldEnabled('ZipCode')) {
	            $fields->push(new TextField("ZipCode", "Zip Code"));
	        }
	        if ($this->getPointer()->isFieldEnabled('Email')) {
	            $fields->push(new EmailField("Email", "Email Address"));
	        }
	        if ($this->getPointer()->isFieldEnabled('Phone')) {
	            $fields->push(new TextField("Phone", "Phone Number"));
	        }
	        if ($this->getPointer()->isFieldEnabled('Comments')) {
	            $fields->push(new TextareaField("Comments", "Comments"));
	        }
